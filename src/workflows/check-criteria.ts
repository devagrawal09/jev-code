...
export interface CheckCriteriaInput {
  criteria: string;
  criteriaSource: string;
  testResults?: { text: string; source: string } | null;
  scope?: DiffScope;
  base?: string;
...
export function parseCriteria(text: string): Criterion[] {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!match?.[1]) continue;
    const value = match[1];
    if (value.length > CRITERIA_POLICY.maxCriterionChars) {
      throw new InputError(
        `criterion on line ${index + 1} is ${value.length} characters; the limit is ${CRITERIA_POLICY.maxCriterionChars}`,
      );
    }
  ...
  if (items.length > CRITERIA_POLICY.maxCriteria) {
    throw new InputError(`${items.length} criteria found; the limit is ${CRITERIA_POLICY.maxCriteria}`);
  }
  return items;
}
...
export async function checkCriteria(
  input: CheckCriteriaInput,
  options: RunOptions,
): Promise<Packet<CriterionResult>> {
  ...
  if (diff.hunks.length === 0) limits.push("the diff is empty; no evidence units exist");
  ...
  for (const criterion of criteria) {
    ...
    for (const [index, hunk] of judgedUnits.entries()) {
      ...
      if (p >= CRITERIA_POLICY.accept) accepted.push({ hunk, p });
      else if (inBand(p, CRITERIA_POLICY.unsupportedBelow, CRITERIA_POLICY.accept)) band.push({ hunk, p });
    }
    ...
    if (accepted.length === 0) {
      ...
      await run.decision(criterion.id, "no_accepted_evidence", result.status, CRITERIA_POLICY.version);
      continue;
    }
    strengthJobs.push({
      result,
      criterion,
      evidence: accepted.slice(0, CRITERIA_POLICY.maxEvidencePerCriterion).map((e) => e.hunk),
      linked,
    });
  }
  ...
  for (const [index, { result, criterion, linked }] of strengthJobs.entries()) {
    ...
    const level = answer.probabilities[best]! >= CRITERIA_POLICY.decisiveLevel ? best : null;
    result.strength = {
      level,
  ...
  return run.finish({
    ...
    notChecked: [
      ...
      "tests were not executed; only supplied test records are considered",
