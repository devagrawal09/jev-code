...
export interface CandidateChecks {
  /** The candidate directory contained exactly one loadable plugin. */
...
export interface CandidateRecord {
  ...
  /** Repository-relative path of the plugin file or package directory inside the candidate directory. */
...
export interface ImprovementContext {
  ...
  /** Repository-relative directory of active plugins, offered as examples. */
  readonly pluginDirectory: string;
  /** Repository-relative paths of currently active plugins. */
  readonly existingPlugins: readonly string[];
  /** Workflow ids already taken by built-ins and plugins. */
  readonly reservedIds: readonly string[];
  /** Source of a complete reference workflow in the required shape (see `examples/plugins/`). */
...
export function improvementInstructions(job: ImprovementJob, context: ImprovementContext): string {
  const examples =
    context.existingPlugins.length > 0
      ? [
          "Active workflows in this repository (read them as examples of the same API):",
          ...context.existingPlugins.map((path) => `  - ${path}`),
        ]
      : [`There are no repository workflows yet under ${context.pluginDirectory}/.`];
  return [
    ...
    "",
    "Deliverable:",
    `- Write exactly one plugin file at ${context.candidateDirectory}/<workflow-id>.ts (create the directory if`,
    ...
    "",
    "Workflow contract (Stanley plugin API):",
    "- The module default-exports an async factory: `export default async ({ root, signal, log }) => plugin`.",
    "- The plugin object needs `id` (lowercase, /^[a-z][a-z0-9_-]{0,63}$/) and `async run(context)`.",
    ...
    "- Plugins are trusted repository code and may use Node APIs, but must stay inside `root`, must never",
